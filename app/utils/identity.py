...
async def validate_auth(*args, **kwargs):
    try:
        token = get_token_auth_cookie(kwargs["request"])
        url = f'https://login.microsoftonline.com/{settings.tenant_id}/discovery/v2.0/keys'
...
def requires_auth(f):
    ...
    async def decorated(*args, **kwargs):
        response = await validate_auth(*args, **kwargs)
        if response:
            return response
        else:
...
def __decode_JWT(token_version, token, rsa_key):
    if token_version == "1.0":
        _issuer = f'https://sts.windows.net/{settings.tenant_id}/'
        _audience=f'api://{settings.app_id}'
    else:
        _issuer = f'https://login.microsoftonline.com/{settings.tenant_id}/v2.0'
        _audience=f'{settings.app_id}'
    try:
        payload = jwt.decode(
            token,
            rsa_key,
